...
// }
  ...
  // urlDemoTest() {
  //   String urld =
  //       "https://esmartpos.com/eprint/posprint.php/eprint?sale_id=2095&print=inv&token=627289&endpoint=iresto_dev&kot_for_item_modified=0&is_print_booking=0";
  //   // "https://pos.tradywork.com/IR_api/get_sale_details?sale_id=92236&&action=kitchen&&outlet_id=21&&company_id=26";

  //   var response = dio.get(urld);
...
// });
  ...
  void submitStudent(BuildContext context) {
    if (urltext.text.contains("https://irestoraplus.easypos4u.com") ||
        urltext.text.contains("https://app.esmartpos.com")) {
  ...
  var toggle = false;
  var logStatus = '';
  Future<void> kotPrintReceiptFormat(
      NetworkPrinter printer, var kotDataList) async {
    for (int j = 0; j < restaurant.data.length; j++) {
      if (restaurant.data[j].type == "header") {
        ...
        // printer.text("");
        for (int y = 0; y < datalist.subTitles.length; y++) {
          printer.row([
            PosColumn(
    ...
    for (int r = 0; r < kotDataList.itemdata.length; r++) {
      ...
      printer.row([
        PosColumn(
            text: kotDataList.itemdata[r].quantity.toString() +
                " x " +
                printsplit[0],
            width: 8,
            styles: PosStyles(
      ...
      for (int l = 0; l < kotDataList.itemdata[r].toppings.length; l++) {
        printer.row([
          PosColumn(
              text: "  " + kotDataList.itemdata[r].toppings[l],
              width: 10,
              styles:
                  PosStyles(align: PosAlign.left, width: PosTextSize.size1)),
          PosColumn(
      ...
      for (int m = 0; m < kotDataList.itemdata[r].items.length; m++) {
        ...
        for (int n = 0;
            n < kotDataList.itemdata[r].items[m].toppings.length;
            n++) {
          printer.row([
            PosColumn(
                text:
                    "  " + "x " + kotDataList.itemdata[r].items[m].toppings[n],
                width: 10,
                styles:
                    PosStyles(align: PosAlign.left, width: PosTextSize.size1)),
            PosColumn(
    ...
    for (int j = 0; j < restaurant.data.length; j++) {
      ...
      if (restaurant.data[j].type == "item") {
        ...
        for (int k = 0; k < datalistitem.itemdata.length; k++) {
          ...
          for (int l = 0; l < datalistitem.itemdata[k].toppings.length; l++) {
            // var printsplit =
            //     splitByLengthSign(datalistitem.itemdata[k].toppings[l], 30);
            var str = datalistitem.itemdata[k].toppings[l];
            var parts = str.split('~');
            var startTopping = parts[0].trim(); // prefix: "date"
            var endToppings = parts.sublist(1).join('~').trim();
            printer.row([
              PosColumn(
                  text: "  " + startTopping,
      ...
      if (restaurant.data[j].type == "footer") {
        ...
        for (int d = 0; d < footerlist.footerText.length; d++) {
          var printsplit = splitByLengthFotter(footerlist.footerText[d], 42);
          //     printer.row([
          //   PosColumn(
          ...
          // ]);
          for (int f = 0; f < printsplit.length; f++) {
            printer.row([
              PosColumn(
                  text: printsplit[f].replaceAll("\n", ""),
                  //printsplit.first.replaceAll("\n", ""),
                  //footerlist.footerText[d].replaceAll("\n", ""),
                  width: 11,
                  styles: PosStyles(
                      // bold: true,
                      align: PosAlign.left,
  ...
  List<String> splitByLengthFotter(String value, int length) {
    List<String> pieces = [];
  ...
  List<String> splitByLengthSign(String value, int length) {
    List<String> pieces = [];
  ...
  void getKotPrintIp(
      String printerIp, BuildContext ctx, var kichenDataList) async {
  ...
  initSavetoPath() async {
    ...
    var bytes = await rootBundle.load("asset/ssh.png");
                              ...
                              children: [
                                Padding(
                                  ...
                                  child: TextFormField(
                                    ...
                                    //   }
                                    //   if (!val.contains(
                                    //       "https://irestoraplus.easypos4u.com")) {
                                ...
                                try {
                                  ...
                                  bluetooth.isConnected
                                    ...
                                    if (isConnected) {
                                            ...
                                            for (int l = 0;
                                                l <
                                                    datalistitem.itemdata[k]
                                                        .toppings.length;
                                                l++) {
                                              var str = datalistitem
                                                  .itemdata[k].toppings[l];
                                              var parts = str.split('~');
                                              var prefix = parts[0]
                                                  .trim(); // prefix: "date"
                                              var date = parts
                                      ...
                                      //                                         j < restaurant.data.length;
                                      //                                         j++){
                                      //  df = restaurant.data[j].type.contains("kitchen_print");
                                      //                                     print(df);
                                      //                                     setState(() {
                                      //                                        kitchenlength = restaurant.data[j];
                                      //                                     });
                                      ...
                                      for (int j = 0;
                                        ...
                                        if (restaurant.data[j].type == "item") {
                                          try {
                                            if (localdatalist != 0) {
                                              lanPrint(localdatalist.last.ip,
                                                  context);
                                              return null;
                                            } else {
                                              return null;
                                      ...
                                      // lanPrint(localdatalist.last.ip, context);
                                    }
                                  });
                                } catch (e) {
                                  saveLogFile(
